...
from utils.custom_funcs import (
    dice_coefficient,
    combined_loss,
    combined_loss_bayesian_unet,
...
def train_unet(train_dataset, test_dataset):
    ...
    if MIXED_PRECISION:
        policy = tf.keras.mixed_precision.Policy(
            "mixed_float16"
        )  # mixed precisions means using 16-bit floating point numbers where possible
    ...
    checkpoint = ModelCheckpoint(
        f"checkpoints/unet_model_{DROPOUT_RATE}_{ACTIVATION_FUNC}.keras",
    ...
    model.fit(
        ...
        callbacks=[checkpoint, tensorboard, csv_logger, learning_rate_scheduler],
        verbose=2,
    )
...
def train_bayesian_unet(train_dataset, test_dataset):
    if Path(
        f"checkpoints/bayesian_unet_model_{DROPOUT_RATE}_{ACTIVATION_FUNC}.h5"
    ).exists():
        logger.info("Model already exists, exiting...")
        pass
    ...
    model.compile(
        optimizer="adam",
        loss=combined_loss_bayesian_unet,
