...
if __name__ == '__main__':
    ...
    for i in range(50, 60):
        mc_predictions = mc_dropout_predictions(model, test_images[i], num_samples=NUM_SAMPLES_MC_DROPOUT_PREDICTION)
        mean_prediction = np.mean(mc_predictions, axis=0)
        std_deviation = np.std(mc_predictions, axis=0)
        visualize_mean_std(test_images[i], test_masks[i], mean_prediction, std_deviation)
        visualize_confidence_intervals(test_images[i], mean_prediction, std_deviation, confidence_level=0.95)
