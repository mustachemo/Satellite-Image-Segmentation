...
def run_mc_dropout_on_all_images(model, images, num_samples=10):
    ...
    for image in tqdm(
        images,
        desc=f"MC Dropout Inference for all test images ({num_samples} times each)",
    ):
...
def mc_dropout_predictions(model, input_image, num_samples=50):
    ...
    predictions = [
        ...
        for _ in tqdm(
            range(num_samples), desc="MC Dropout Inference for a single test image"
        )
    ]
...
def visualize_mean_std_grid_multi_models(
    test_images, test_masks, predictions, rows=4, cols=5, titles=None
):
    """
    Visualize the test images, masks, mean predictions, and uncertainty across multiple samples.
...
def visualize_confidence_intervals(
    test_image, mean_prediction, std_deviation, confidence_level=0.95
):
