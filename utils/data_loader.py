...
def process_image(
    ...
    try:
        image = load_img(
            ...
            target_size=(X_DIMENSION, Y_DIMENSION),
        )
        mask = load_img(
            mask_path,
...
def load_and_process_files(image_dir, mask_dir, prefix="train"):
    ...
    with ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(
                process_image, image_path, mask_dir / f"{image_path.stem}_mask.png"
            ): image_path
        ...
        for future in tqdm(
            ...
            total=len(futures),
            desc=f"Loading and processing {prefix} data",
        ):
...
def write_tfrecord(filename, images, masks):
    with tf.io.TFRecordWriter(filename) as writer:
        with ThreadPoolExecutor() as executor:
            futures = [
                executor.submit(serialize_example, image, mask)
                for image, mask in zip(images, masks)
            ]
            for future in tqdm(
                as_completed(futures), total=len(futures), desc="Writing TFRecord"
            ):
...
def create_tf_dataset_from_tfrecord(tfrecord_files):
    ...
    logging.info(
        f"Creted dataset from {tfrecord_files} with batch size {BATCH_SIZE}..."
    )
